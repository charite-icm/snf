...
import numpy as np
import pytest
from src.snf_package import compute
from src.snf_package import helper_functions
...
def test_dnorm(norm):
    helper_functions._dnorm(rs.rand(100, 100), norm=norm)
    with pytest.raises(ValueError):
        helper_functions._dnorm(rs.rand(100, 100), norm='bad')